	...
	init(negative) {
		...
		template = [
			...
			},
			{
				label: 'Edit',
				submenu: [
					{ label: 'Undo',  accelerator: 'Command+Z',       click: negative.undo,  enabled: false },
					{ label: 'Redo',  accelerator: 'Shift+Command+Z', click: negative.redo,  enabled: false },
					MENU_SEPARATOR,
					{ label: 'Copy',  accelerator: 'Command+C',       click: negative.copy,  enabled: false },
					{ label: 'Paste', accelerator: 'Command+V',       click: negative.paste, enabled: false }
				]
			},
			{
				label: 'View',
				submenu: [
					{ label: 'Capture',         accelerator: 'Command+G',         click: negative.captureRegionBehindWindow },
					{ label: 'Clear',           accelerator: 'Command+Backspace', click: negative.removeImage,     enabled: false },
					MENU_SEPARATOR,
					{ label: 'Actual Size',     accelerator: 'Command+0',         click: () => negative.zoomTo(1), enabled: false },
					{ label: 'Zoom In',         accelerator: 'Command+=',         click: negative.zoomIn,          enabled: false },
					{ label: 'Zoom Out',        accelerator: 'Command+-',         click: negative.zoomOut,         enabled: false },
					MENU_SEPARATOR,
			...
			},
			{
				...
				submenu: [
					...
					{ label: 'Next Tab',                accelerator: 'Command+}',     click: negative.selectNextTab,                              enabled: false },
					{ label: 'Previous Tab',            accelerator: 'Command+{',     click: negative.selectPreviousTab,                          enabled: false },
					{ label: 'Next Tab and Resize',     accelerator: 'Alt+Command+}', click: negative.selectNextTabAndResizeWindowToFitImage,     enabled: false },
					{ label: 'Previous Tab and Resize', accelerator: 'Alt+Command+{', click: negative.selectPreviousTabAndResizeWindowToFitImage, enabled: false },
					{
						label: 'Move',
						submenu: [
					...
					for (const submenuItem of item.submenu) {
						switch (submenuItem.label) {
							...
							case 'Previous Tab and Resize':
								submenuItem.enabled = menuStates.canInteractWithTabs;
								break;
							case 'Move':
