...
module.exports = {
	...
	createMenu(menuStates, isSettingsWindow) {
		...
		menuStates             = Object.assign({}, DEFAULT_MENU_STATES, lastMenuStates, menuStates);
		...
		lastMenuStates         = menuStates;
		isSettingsWindow       = !!isSettingsWindow;
		
		if (NEGATIVE_VERBOSE === 'true') {
			/* eslint-disable no-console */
			console.log(`\tisSettingsWindow = ${isSettingsWindow}`);
			console.log(`\tDEFAULT_MENU_STATES = ${JSON.stringify(DEFAULT_MENU_STATES, null, '\t')}`);
			console.log(`\tlastMenuStates = ${JSON.stringify(lastMenuStates, null, '\t')}`);
			console.log(`\tmenuStates = ${JSON.stringify(menuStates, null, '\t')}\n`);
			/* eslint-enable no-console */
		}
	...
	refreshForNewWindow() {
		...
		this.createMenu(DEFAULT_MENU_STATES, false);
	},
