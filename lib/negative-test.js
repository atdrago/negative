...
module.exports = {
	init() {
		ipcMain.on('test-new-tab', () => this.simulateMenuClick(['File', 'New Tab']));
		
		ipcMain.on('test-capture', () => this.simulateMenuClick(['View', 'Capture']));
		ipcMain.on('test-clear', () => this.simulateMenuClick(['View', 'Clear']));
