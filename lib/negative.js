...
module.exports = {
	init() {
		...
		this.negativeSettings.init(() => {
			...
			if (!negativeWindows || negativeWindows.length === 0 || process.env.NEGATIVE_IGNORE_WINDOW_SETTINGS === 'true') {
	...
	quit() {
		...
		console.log('NEGATIVE -- negative.quit called');
		...
		if (windowCount > 0) {
			ipcMain.on('window-settings-response', (evt, _window) => {
				...
				if (negativeWindows.length === windowCount) {
					...
					this.negativeSettings.save(() => {
						console.log('NEGATIVE -- app.quit called');
						app.quit()
					});
				}
			});
			...
			for (const key of windowsKeys) {
				...
				windowSettings.isFocused = (key === mainWindowId);
				
				_window.webContents.send('window-settings-request', windowSettings);
			}
		} else {
			this.negativeSettings.set('windowSettings', []);
			this.negativeSettings.save(() => {
				console.log('NEGATIVE -- app.quit called');
				app.quit()
			});
	...
	reset() {
		...
		if (process.env.NEGATIVE_SKIP_RESET_DIALOG === 'true') {
			_reset();
		} else {
