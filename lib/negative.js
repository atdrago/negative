...
module.exports = {
	init() {
		...
		this.negativeSettings.init(() => {
			const windowSettings = this.negativeSettings.get('windowSettings');
			
			if (!windowSettings || windowSettings.length === 0 || NEGATIVE_IGNORE_SETTINGS === 'true') {
				this.initNegativeWindow();
			} else {
				windowSettings.forEach((windowState) => this.initNegativeWindow(windowState));
			}
		});
		...
		app.on('before-quit', () => {
			...
			if (windowCount > 0) {
				for (const key of windowsKeys) {
					...
					windowSettings.push(windowState);
				}
			}
			
			this.negativeSettings.set('windowSettings', windowSettings);
	...
	getNewNegativeWindow(windowState) {
		const bounds = windowState ? windowState.bounds : {};
	...
	initNegativeWindow(windowState) {
		const newWindow = this.getNewNegativeWindow(windowState);
		const isFocused = windowState && windowState.isFocused !== false;
