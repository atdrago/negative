...
module.exports = {
	init() {
		...
		negativeSettings.init(() => {
			const negativeWindows = negativeSettings.get('negative.windows');
			
			if (!negativeWindows || negativeWindows.length === 0) {
				this.initNegativeWindow();
			} else {
				for (const _window of negativeWindows) {
					this.initNegativeWindow(_window);
				}
			}
		});
	...
	quit() {
		const windows         = electronWindow.windows;
		const windowCount     = Object.keys(windows).length; // @TODO - need to account for settings window
		...
		for (const key in windows) {
			if (windows.hasOwnProperty(key) && settingsWindowId != key) {
				...
				_window.webContents.executeJavaScript('negative.sendUndoManagersToMain()');
				ipcMain.on('undo-managers-response', (evt, undoManagers) => {
					negativeWindows.push(undoManagers);
					
					if (negativeWindows.length === windowCount) {
						negativeSettings.set('negative.windows', negativeWindows);
	...
	initNegativeWindow(_window) {
		const undoManagers = _window;
		
		console.log(this);
		
		const newWindow = this.getNewNegativeWindow();
		...
		if (!undoManagers) {
			...
			undoManagers.forEach((undoManager) => {
				...
				// newWindow.webContents.executeJavaScript(`negative.addTabForUndoManager(${undoManagerString});`);
			});
			newWindow.webContents.executeJavaScript(`
				${addTabsString}
