...
module.exports = {
		...
		ipcMain.on('set-settings-request', (evt, settings) => {
			...
			for (const key in windows) {
				if (windows.hasOwnProperty(key) && settingsWindowId !== key) {
					...
					shouldAutoUpdate = this.negativeSettings.get('shouldAutoUpdate');
					shouldShowTips = this.negativeSettings.get('shouldShowTips');
	...
	initNegativeWindow(_window) {
		...
		newWindow.showUrl(path.resolve(__dirname, '../view/index.html'), { isDarkMode: isDarkMode, shouldShowTips :shouldShowTips }, () => {
	...
	initSettingsWindow(callback) {
		if (!settingsWindowId) {
			...
			settingsWindow.showUrl(path.resolve(__dirname, '../view/settings.html'), { shouldAutoUpdate: shouldAutoUpdate, shouldShowTips: shouldShowTips }, () => {
