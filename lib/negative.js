...
module.exports = {
    ...
    captureRegionBehindWindow() {
        ...
        //     cgImage = CGWindowListCreateImage(rect, .OptionOnScreenBelowWindow, windowId, .Default),
        //     ciImage = CIImage(CGImage: cgImage!),
        //     ciFilter = CIFilter(name: "CIColorInvert", withInputParameters: [kCIInputImageKey: ciImage]),
        //     outputCiImage = ciFilter?.valueForKey(kCIOutputImageKey) as! CIImage,
        //     ciContext = CIContext(options: [
        //         kCIContextWorkingColorSpace: NSNull(),
        //         kCIContextUseSoftwareRenderer: NSNumber(bool: true)
        //     ]),
        //     outputCgImage = ciContext.createCGImage(outputCiImage, fromRect: ciImage.extent),
        //     nsImage = NSImage(CGImage: outputCgImage, size: rect.size);
        ...
        if (focusedWindow) {
            ...
            let rect = objc.NSMakeRect(newImageX, newImageY, newImageWidth, newImageHeight);
            let windowId = objc.NSApplication('sharedApplication')('keyWindow')('windowNumber');
            let cgImage = objc.CGWindowListCreateImage(rect, objc.kCGWindowListOptionOnScreenBelowWindow, windowId, objc.kCGWindowImageDefault);
            let nsImage = objc.NSImage('alloc')('initWithCGImage', cgImage, 'size', rect.size);
            let screencapturePath = `${app.getPath('temp')}negative-sc.tiff`;
            nsImage('TIFFRepresentation')('writeToFile', objc(screencapturePath), 'atomically', false)

            console.log(screencapturePath)
