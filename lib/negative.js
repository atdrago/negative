...
module.exports = {
	...
	initAutoUpdater() {
		...
		autoUpdater.on('error', (err) => {
			this.executeJavaScriptInSettingsWindow(`
				if (window.settingsController) {
					window.settingsController.hideLoadingIndicator();
					window.settingsController.enableUpdatesButton();
					window.settingsController.setUpdatesMessage('${err}');
				}
			`);
		});
		...
		autoUpdater.on('checking-for-update', () => {
			this.executeJavaScriptInSettingsWindow(`
				if (window.settingsController) {
					window.settingsController.disableUpdatesButton();
					window.settingsController.showLoadingIndicator();
					window.settingsController.setUpdatesMessage('Checking for update...');
				}
			`);
		});
		...
		autoUpdater.on('update-available', () => {
			this.executeJavaScriptInSettingsWindow(`
				if (window.settingsController) {
					window.settingsController.disableUpdatesButton();
					window.settingsController.showLoadingIndicator();
					window.settingsController.setUpdatesMessage('Downloading update...');
				}
			`);
		});
		
		autoUpdater.on('update-not-available', () => {
			this.executeJavaScriptInSettingsWindow('window.settingsController.hideLoadingIndicator();');
		});
		...
		autoUpdater.on('update-downloaded', () => {
			this.executeJavaScriptInSettingsWindow(`
				if (window.settingsController) {
					window.settingsController.enableUpdatesButton();
					window.settingsController.hideUpdatesButton();
					window.settingsController.hideLoadingIndicator();
					window.settingsController.showRestartAndInstallButton();
					window.settingsController.setUpdatesMessage('');
				}
			`);
		});
	},
