...
describe('Window > Next Tab And Resize', function () {
	...
	it('Should go to next tab and resize', function () {
		let firstTabBounds;
		...
		return this.app.client.waitUntilWindowLoaded()
			...
			.then((bounds) => {
				...
				return this.app.electron.ipcRenderer.send('test-next-tab-and-resize');
			})
			.then(() => {
			...
			.then(() => this.app.electron.ipcRenderer.send('test-next-tab-and-resize'))
			.then(() => {
				return this.app.client.waitUntil(() => {
					return this.app.browserWindow.getBounds()
						.then((bounds) => {
